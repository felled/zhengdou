<?php
/* template name: 创意科技
description: template for Git theme
*/ get_header();?>
<title>创意科技</title>
<div class="content height">
    <div class="join-conten hover-blur">
        <div class="news-title join-title">
            <h2>创 意  科 技</h2>
            <span>CREATIVE TECHNOLOGY</span>
        </div>
        <div class="creat-info">
            <div>
                <a href="/">如此高逼格的挂钟你见过吗？</a>
                <a href="/">结满果实的树枝灯？</a>
                <a href="/">沉浸式技术将在2019年有巨大发展？</a>
                <a href="/">闻得到的VR，你想体验吗？？</a>
                <a href="/">工程投影助力全息投影迸发惊人生命力？</a>
                <a href="/">Black 13 park辉煌五周年大盘点？</a>
                <a href="/">开天MOSS电影道具复刻雕像？</a>
                <a href="/">SK糖王 | 翻糖人偶作品 《精卫》 ？</a>
            </div>
            <div class="pages">
                <span class="prev">第一页</span>
                <em class="line"></em>
                <span class="next">下一页</span>
            </div>
            <img class="sub-mb-img" src="<?php bloginfo('template_url'); ?>/images/creative/mb/bottom-img.png">
        </div>
    </div>
    <img class="left-img creat-img blur-img" src="<?php bloginfo('template_url'); ?>/images/creative/pc/creat-img.png">
    <img class="right-img creat-img blur-img" src="<?php bloginfo('template_url'); ?>/images/creative/pc/creat-img.png">
</div>
<div class="footer">
    <div class="footer-info">
        <a href="/contact" target="_blank">联系我们</a>
        <em>/</em>
        <a href="/join" target="_blank">加入我们</a>
        <em>/</em>
        <a  class="law">法律声明</a>
    </div>
</div>
<div class="right-fox right-fox-right">
    <div class="right-info">
        <h2>版权声明</h2>
        <p>本网站（www.szcubean.com）上为版权人所有的文件，在此并未作任何授权，本网站上的文件仅限于为信息和非商业或个人之目的使用，并且不得在任何网络计算机上复制或公布，也不得在任何媒体上传播。未经版权人许可，任何人不得擅自（包括不限于：以非法的方式复制、传播、展示、镜像、上传、下载）使用。否则版权人将依法追究其法律责任。</p>
        <p>免责声明：版权人拥有对本网站的内容进行随时更改之权利，事前将不会另行通知。版权人不保证或声明本网站展示的资料是否正确、全面或是最新教程。本网站的所有图片、模型、文字描述等、仅作为参考信息</p>
    </div>
</div>
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/my-js/jquery.min.js"></script>
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/my-js/public.js"></script>
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/my-js/project.js"></script>
<?php  ?>


